...
//        'paper' — generates a 1-page clinical research paper on a given topic
//
// Required env var: ANTHROPIC_API_KEY (set in Netlify → Site settings → Environment variables)
...
exports.handler = async function(event) {
  ...
  if (!apiKey) return { statusCode: 500, headers, body: JSON.stringify({ error: 'API key not configured. Set ANTHROPIC_API_KEY in Netlify environment variables.' }) };
  ...
  // Paper mode uses Haiku 4.5 (3-5x faster than Sonnet) to fit Netlify free-plan 10s timeout.
  // Chat mode keeps Sonnet 4.6 — answers are short (<800 tokens) so well within 10s.
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { statusCode: response.status, headers, body: JSON.stringify({ error: 'Anthropic API error: ' + errText.slice(0, 200) }) };
    }

    const data = await response.json();
    const reply = data.content && data.content[0] && data.content[0].text ? data.content[0].text : 'No response generated.';
    return { statusCode: 200, headers, body: JSON.stringify({ reply, mode }) };
  } catch (err) {
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Server error: ' + (err.message || 'unknown') }) };
  }
};
